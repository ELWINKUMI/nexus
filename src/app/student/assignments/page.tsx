...
export default function StudentAssignmentsPage() {
  ...
  // Ensure client-side mounting to prevent hydration issues
  useEffect(() => {
    setIsMounted(true);
  }, []);
  ...
  const loadAssignments = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Use real assignments or fallback to mock data if none exist
        if (data.assignments && data.assignments.length > 0) {
          setAssignments(data.assignments);
        } else {
          console.log('No real assignments found, using mock data');
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
          <Box sx={{ flex: 1 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
              ...
              />
              {(assignment.score && typeof assignment.score === 'object' && assignment.score.score !== undefined) && (
                <Chip
                  label={`${assignment.score.score}/${assignment.score.maxPoints} (${assignment.score.percentage}%)`}
                  size="small"
                  color="success"
                  variant="outlined"
                />
              )}
              {(assignment.score && typeof assignment.score === 'number') && (
                <Chip
              ...
              {(assignment.grade) && (
                <Chip
                  label={`${assignment.grade.score}/${assignment.grade.maxPoints} (${assignment.grade.percentage}%)`}
                  size="small"
                  color="success"
                  variant="outlined"
                />
